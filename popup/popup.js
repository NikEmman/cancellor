...
let a130Depts = parseDepts(
  localStorage.getItem("a130-depts") ?? a130DeptsInput.value,
);
...
(function restoreState() {
  ...
  if (amyStored !== null) {
    ...
    if (amyStored)
      a130AmyWarn.style.display = /^\d{4}$/.test(amyStored) ? "none" : "block";
  }
...
async function extractOperatorData() {
  const [result] = await execute(function () {
    ...
    if (!profileCell)
      return {
        success: false,
        error: "Δεν βρίσκεστε στην εφαρμογή Ταυτοτήτων",
      };
    ...
    if (cells.length < 3)
      return {
        success: false,
        error: "Λάθος σελίδα ή η σελίδα Ταυτοτήτων δεν φόρτωσε σωστά",
      };
  ...
  try {
    ...
    for (const [index, id] of runIds.entries()) {
      ...
      try {
        ...
        // Early exit: first issuance (no old ID) or non-standard replacement reason
        if (
          !personData.oldId ||
          personData.replacementReason !== "ΑΝΤΙΚΑΤΑΣΤΑΣΗ ΠΑΛΑΙΟΥ ΤΥΠΟΥ"
        ) {
          const earlyResult = {
            id: id.number,
            success: true,
